...
public class Exporter {
    ...
    public void run() {
        ...
        if (windowless) {
            ...
            if (noLookupTables == null)
                noLookupTables = false;
        }
        ...
        try (IFormatWriter w = new ImageWriter().getWriter(outfile)) {
            ...
            String xml = fi == null ? null
                    : fi.description == null ? null : fi.description.indexOf("xml") == -1 ? null : fi.description;
            ...
            try {
                ...
                if (ptype != originalType && (store.getPixelsType(0) == null || !FormatTools.isSigned(originalType)
                        || FormatTools.getBytesPerPixel(originalType) != FormatTools.getBytesPerPixel(ptype))) {
                ...
                for (int t = 0; t < (splitT ? sizeT : 1); t++) {
                    for (int z = 0; z < (splitZ ? sizeZ : 1); z++) {
                        for (int c = 0; c < (splitC ? sizeC : 1); c++) {
                            int index = FormatTools.getIndex(ORDER, sizeZ, sizeC, sizeT, sizeZ * sizeC * sizeT, z, c,
                                    t);
                            String pattern = base + (splitZ ? "_Z%z" : "") + (splitC ? "_C%c" : "")
                                    + (splitT ? "_T%t" : "") + ext;
            ...
            for (int i = start; i < end; i++) {
                ...
                if (doStack) {
                    ...
                    fileIndex = FormatTools.getIndex(ORDER, realZ, realC, realT, realZ * realC * realT, coords[0],
                            coords[1], coords[2]);
                }
